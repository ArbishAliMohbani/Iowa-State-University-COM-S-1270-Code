# Arbish Ali Mohbani                01/04/2025
# Assignment # 4
# [COM S 1270-1]
# The code implements a console-based "Four In Sequence" game (similar to Connect Four), where players take turns dropping pieces into columns on a grid. The game features both single-player mode against a computer opponent and two-player mode. The computer AI has multiple levels of decision-making: it prioritizes winning moves, blocks opponent's winning moves, tries to connect its pieces, and randomly selects columns as a last resort. The grid-based gameplay tracks horizontal, vertical, and diagonal sequences of four matching pieces to determine a winner, with a draw occurring when the board is completely filled.
...
def main():
    ...
    while running:
        ...
        if choice == "p":
            ...
            print("\nInstructions for Four In Sequence:")
            print("1. The goal is to be the first player to connect four of your pieces in a row - horizontally, vertically, or diagonally.")
            print("2. Players take turns dropping their pieces (X or O) into one of the columns.")
            print("3. The piece will fall to the lowest available position in the selected column.")
            print("4. Player 1 (X) goes first, then Player 2 (O), and so on.")
            print("5. In single player mode, you play against the computer (Player 2).")
            print("6. The game ends when a player connects four pieces or the board is completely filled (draw).")
            print("\nGood luck!\n")
        elif choice == "q":
            print("\nThank you for playing Four In Sequence! Goodbye!")
            running = False
        else:
